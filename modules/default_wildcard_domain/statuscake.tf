...
resource "statuscake_ssl_check" "rancher" {
    ...
    monitored_resource {
        address = "https://${var.rancher_a_record_name}.${var.root_domain}"
    }
}

resource "statuscake_uptime_check" "rancher" {
    name = "cloudcli-prod-rancher"
    check_interval = 60
    confirmation = 3
    http_check {
